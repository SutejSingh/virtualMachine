...
namespace Catch {
	...
	GitHubReporter::GitHubReporter(ReporterConfig const& config)
		...
		if (githubFileName) {
			m_markdownFile.open(githubFileName, std::ios_base::app);
		}
		else {
			m_markdownFile.open("GradeReport.md", std::ios_base::app);
		}
	...
	GitHubReporter::~GitHubReporter() {
		...
		if (m_maxScore == m_totalScore) {
			...
			m_markdownFile << "You received all " << m_maxScore << " points for this part.\n\n";
		}
		else {
			m_markdownFile << "\n## \xF0\x9F\x9A\xA8\xF0\x9F\x9A\xA8 Some test cases failed!! \xF0\x9F\x98\xAD\xF0\x9F\x98\xAD\n\n";
			m_markdownFile << "You received " << m_totalScore << " out of " << m_maxScore << " points for this part.\n\n";
		}
	}
